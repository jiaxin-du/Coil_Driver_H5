...
#define DAC_CMD_WRITE               ((uint8_t)0x10)  // Write to the input register
#define DAC_CMD_WRITE_CHNA          ((uint8_t)0x11)  // Write to the input register of Channel A
#define DAC_CMD_WRITE_CHNB          ((uint8_t)0x18)  // Write to the input register of Channel B
#define DAC_CMD_WRITE_ALL           ((uint8_t)0x19)  // Write to the input register of Channel A and Channel B

#define DAC_CMD_WRITE_UPDATE        ((uint8_t)0x30)  // Write to and update (power up) DAC
#define DAC_CMD_WRITE_UPDATE_CHNA   ((uint8_t)0x31)  // Write to and update (power up) DAC channel A
#define DAC_CMD_WRITE_UPDATE_CHNB   ((uint8_t)0x38)  // Write to and update (power up) DAC channel B
#define DAC_CMD_WRITE_UPDATE_ALL    ((uint8_t)0x39)  // Write to and update all DAC channels
...
// DAC coefficient
float gDAC_Coeff = 13107.2f; // ==2^16/(2 * 2.5)
...
/* this function takes about ~800 cycle (3.2 us)*/
...
void Dac_Set(uint8_t x_chn, float x_volt)
{
   ...
   uint32_t tmp_dac_reg = ((uint32_t) (0.5f + gDAC_Coeff * x_volt) & 0x0000FFFF);
...
/* this function takes about ~160 cycle (0.65 us)*/
// Set a DAC channel to output a DC value
// @arg x_chn: DAC_CH_VOLT or DAC_CH_CURR
// @arg x_val: 0 to 5V (output voltage in Volts)
...
{
   if (x_volt >= 0. && x_volt <= 120) {
      Dac_Set(DAC_CH_VOLT, (x_volt - 4.99) * 0.0332975) ; //coil power = 4.99 + 32 * V_ctrl
...
{
   if (x_volt >= 0. && x_volt <= 120) {
      Dac_Preload(DAC_CH_VOLT, (x_volt - 4.99) * 0.0332975) ; //coil power = 4.99 + 32 * V_ctrl
      gHV_State.volt_set = x_volt;
      return ST_OK;
   }
   else {
...
TState Dac_Curr_Ctrl_Set(float x_curr)
{
   if (x_curr >= 0. && x_curr <= 20.) {
      Dac_Set(DAC_CH_CURR, 0.1 * x_curr); // current gain = 10A/V
...
{
   if (x_curr >= 0. && x_curr <= 20.) {
      Dac_Preload(DAC_CH_CURR, 0.1 * x_curr); // current gain = 10A/V
      gHV_State.curr_set = x_curr;
      return ST_OK;
   }
   else {
