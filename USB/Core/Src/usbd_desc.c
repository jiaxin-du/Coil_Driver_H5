...
uint8_t *USBD_CDC_UserStrDescriptor(USBD_SpeedTypeDef speed, uint8_t idx, uint16_t *length);
...
static void Get_SerialNum(void)
{
  ...
  deviceserial0 = gUID[0]; //*(uint32_t *)DEVICE_ID1;
  deviceserial1 = gUID[1]; //*(uint32_t *)DEVICE_ID2;
  deviceserial2 = gUID[2]; //*(uint32_t *)DEVICE_ID3;
